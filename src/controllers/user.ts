...
import { insertUser,getUsers,getUser,updateUser,deleteUser,getSubjects} from "../services/user";
...
const getAsignaturas=async({params}:Request,res:Response)=>{
    try{
        const {idUser}=params;
        const response=await getSubjects(idUser);
        res.send(response);
    } catch(e){
...
export{getPerson,getPeople,postPerson,updatePerson,deletePerson,getAsignaturas};